...
ALTER TABLE `catalog_product_entity_datetime`
    ADD COLUMN IF NOT EXISTS `entity_id` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Entity ID';
...
ALTER TABLE `catalog_product_bundle_option`
    ADD COLUMN IF NOT EXISTS `new_parent_id` INT(10) UNSIGNED NOT NULL COMMENT 'Parent ID';
ALTER TABLE `catalog_product_bundle_option_value`
    ADD COLUMN IF NOT EXISTS `new_parent_product_id` INT(10) UNSIGNED NOT NULL COMMENT 'Parent Product ID';
...
ALTER TABLE `downloadable_link`
    ADD COLUMN IF NOT EXISTS `new_product_id` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Product ID';
...
ALTER TABLE `catalog_product_super_link`
    ADD COLUMN IF NOT EXISTS `new_parent_id` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Parent ID';
...
ALTER TABLE `catalog_product_relation`
    DROP FOREIGN KEY IF EXISTS `CATALOG_PRODUCT_RELATION_PARENT_ID_CATALOG_PRODUCT_ENTITY_ROW_ID`,
...
ALTER TABLE `downloadable_link`
    DROP FOREIGN KEY IF EXISTS `DOWNLOADABLE_LINK_PRODUCT_ID_CATALOG_PRODUCT_ENTITY_ENTITY_ID`,
...
ALTER TABLE `downloadable_sample`
    DROP FOREIGN KEY IF EXISTS `DOWNLOADABLE_SAMPLE_PRODUCT_ID_CATALOG_PRODUCT_ENTITY_ENTITY_ID`,
...
ALTER TABLE `catalog_product_bundle_option`
    ...
    DROP FOREIGN KEY IF EXISTS `CAT_PRD_BNDL_OPT_PARENT_ID_CAT_PRD_ENTT_ENTT_ID`,
...
-- Note: CAT_PRD_BNDL_OPT_VAL_OPT_ID_CAT_PRD_BNDL_OPT_OPT_ID FK already exists in this EE database
-- and references bundle_option(option_id) which remains valid after migration — leave it in place.
-- Dropping and re-adding a FK with the same name in one ALTER TABLE causes InnoDB errno 121.
ALTER TABLE `catalog_product_bundle_option_value`
...
-- Note: CAT_PRD_BNDL_SELECTION_OPT_ID_CAT_PRD_BNDL_OPT_OPT_ID and FK_DCF37523AA05D770A70AA4ED7C2616E4
-- already exist in this EE database and remain valid after migration — no drop/re-add needed.
